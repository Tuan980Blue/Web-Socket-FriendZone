    ...
    <div className="flex items-start space-x-2 md:space-x-3">
      ...
      <div className="flex-1">
        <div className="flex justify-between items-center mb-1 md:mb-2">
          <h3 className={`font-semibold text-base md:text-lg ${theme === 'dark' ? 'text-[#FAFAFA]' : 'text-[#262626]'}`}>
            {isExpanded ? 'Tạo bài viết mới' : 'Bạn đang nghĩ gì?'}
          </h3>
          {isExpanded && (
            <button 
              onClick={() => setIsExpanded(false)}
              className={`p-1 md:p-1.5 rounded-full hover:bg-[#FAFAFA] dark:hover:bg-[#262626] transition-colors duration-200`}
            >
              <X size={18} className={theme === 'dark' ? 'text-[#8E8E8E]' : 'text-[#8E8E8E]'} />
            </button>
          )}
        </div>
      </div>
